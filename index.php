...
<body>
  ...
  <?php
    ...
    $db     = pg_connect("host=localhost port=5432 dbname=postgres user=postgres password=CS2102wjc");	
    ...
    if (isset($_POST['submit'])) {
    	...
    	<li><input type='submit' name='new' value='Insert'/></li>
      <li><input type='submit' name='change' value='Update'/></li>  
    	</form>  
    	</ul>";
    }
    if (isset($_POST['new'])) {	// Submit the update SQL command
        $result = pg_query($db, "INSERT INTO book VALUES ( '$_POST[bookid_updated]',  
    '$_POST[book_name_updated]', '$_POST[price_updated]', '$_POST[dop_updated]')");
        if (!$result) {
            echo "Insert failed!!";
        } else {
    ...
    if (isset($_POST['change'])) { // Submit the update SQL command
        $result = pg_query($db, "UPDATE book SET name = '$_POST[book_name_updated]', price = '$_POST[price_updated]',  
